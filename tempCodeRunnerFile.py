...
def CheckSurroundingCells(Board, BoardDimension, Row, Column):  Neighbouring = 0 
    StartRow, EndRow, StartColumn, EndColumn = 
    CheckRange(BoardDimension, Row, Column) 
    for R in range(StartRow, EndRow + 1): 
        if R != Row: 
            CharacterRange = CheckCharacter(Board[R][Column])  if CharacterRange == "Lower Case":
            Neighbouring = Neighbouring + 1 
    for C in range(StartColumn, EndColumn + 1): 
        if C != Column: 
            CharacterRange = CheckCharacter(Board[Row][C])  if CharacterRange == "Lower Case": 
            Neighbouring = Neighbouring + 1 
    return Neighbouring 
  
def MakeMove(Column, Row, Board, BoardDimension): 
 if CheckCharacter(Board[Row][Column]) == "Lower Case":  print("Well done! The cell (" + str(Column) + "," + str(Row) + ") contains treasure.") 
 Board[Row][Column] = Board[Row][Column].upper()  elif CheckCharacter(Board[Row][Column]) == "Digit" or CheckCharacter(Board[Row][Column]) == "Upper Case": 
...
def DisplayBoard(Board, BoardDimension, Hide): 
 ...
 print("{}".format(Board[Row][Column]), end="")  if Column != BoardDimension - 1: 
 print(" {:2}".format("|"), end="") 
 print() 
...
def PlayGame(Board, BoardDimension): 
 ...
 print("You have to score " + str(ScoreToWin) + " points to win.")  print("You can make a maximum of " + str(MaxMoves) + " moves.")  while GameWon == False and Moves < MaxMoves: 
 DisplayBoard(Board, BoardDimension, Hide) 
 Column, Row = GetColumnRow(BoardDimension) 
 Board = MakeMove(Column, Row, Board, BoardDimension)  Moves = Moves + 1 
 Score = CalculateScore(Board, BoardDimension)  if Score >= ScoreToWin: 
 GameWon = True 
 DisplayBoard(Board, BoardDimension, Hide) 
 print() 
 if GameWon: 
 print("You won the game in " + str(Moves) + " moves.")  else: 
...
if __name__ == "__main__": 
 ...
 BoardDimension = 5 
 Board = CreateBoard(BoardDimension) 
 Board = InitialiseRandomBoard(Board, BoardDimension)  PlayGame(Board, BoardDimension) 
 elif MenuOption == "3": 
 BoardDimension = 10 
 Board = CreateBoard(BoardDimension) 
 Board = InitialiseRandomBoard(Board, BoardDimension)   PlayGame(Board, BoardDimension)
